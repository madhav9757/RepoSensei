...
import RepoCard from "@/components/repo/RepoCard"; // ✅ Import RepoCard
import StatCard from "@/components/repo/StatCard";// ✅ Import StatCard
import EmptyState from "@/components/repo/EmptyState"; // ✅ Import EmptyState
...
export default function Dashboard() {
  ...
  const stats = useMemo(() => ({
    ...
    avgScore: repositories.length
      ? Math.round(repositories.reduce((acc, r) => acc + (r.score || 0), 0) / repositories.length)
      : 0
  }), [repositories]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      {error && (
        <Card className="border-red-200 bg-red-50 dark:bg-red-900/20 dark:border-red-800">
          <CardContent className="p-6 flex items-center gap-3">
            ...
            <Button onClick={fetchRepositories} variant="outline" size="sm">Retry</Button>
          </CardContent>
        </Card>
      )}
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          onClear={() => { setSearchQuery(""); setLanguageFilter("all"); }}
        />
      ) : (
        <motion.div
